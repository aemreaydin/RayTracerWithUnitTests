...
namespace RayTracerUnitTest
{
	...
	TEST_CLASS(MatrixUnitTest)
	{
		...
		TEST_METHOD(MatrixTransform)
		{
			...
			XMatrix::Transform(matrix, XVector4(10.0f, 5.0f, 7.0f), XVector4(angle, 0.0f, 0.0f),
			                   XVector4(5.0f, 5.0f, 5.0f), point);
	...
	TEST_CLASS(RayTest)
	{
		TEST_METHOD(RayPointAtTimeT)
		{
			const XRay ray(XVector(2.0f, 3.0f, 4.0f), XVector(1.0f, 2.0f, 3.0f));

			const auto t1 = ray.PointAtTimeT(1.0f);
			const auto t3 = ray.PointAtTimeT(3.0f);

			const auto t2 = ray.PointAtTimeT(-1.0f);
			const auto t4 = ray.PointAtTimeT(-2.0f);


			Assert::AreEqual(t1, XVector(3.0f, 5.0f, 7.0f));
			Assert::AreEqual(t3, XVector(5.0f, 9.0f, 13.0f));

			Assert::AreEqual(t2, XVector(1.0f, 1.0f, 1.0f));
			Assert::AreEqual(t4, XVector(0.0f, -1.0f, -2.0f));
		}
	};
}
